...
from pipe.train_resnet_128 import (
    MelanomaDataset,
    MyModel,
...
name = "resnet"
metric_fpath = metrics_path / "l1_resnet_tta_128_cv.metric"
...
def main(tta_iters=50):
    ...
    for fold_number in range(n_folds):
        ...
        ckpt_fpath = (
            models_path / f"model_name=resnet_sz=128_fold={fold_number}.ckpt"
        )
        ...
        valid_ds = MelanomaDataset(
            df=valid_df,
            images_path=train_img_128_path,  # we are doing OOF CV
            augmentations=get_tta_transforms(),
            train_or_valid=True,
        )
        valid_dl = DataLoader(
            valid_ds,
            batch_size=hparams.bs * 6,
        ...
        # model.test_images_path = train_img_128_path
    ...
    oof_preds.to_csv(data_path / "oof_preds_resnet_tta_128.csv", index=False)
    ...
    for fold_number in range(n_folds):
        ...
        test_ds = MelanomaDataset(
            df=test_df,
            images_path=test_img_128_path,  # we are doing OOF CV
            augmentations=get_tta_transforms(),
    ...
    preds.to_csv(
        submissions_path / "l1_resnet_tta_128_submission.csv", index=False
    )
